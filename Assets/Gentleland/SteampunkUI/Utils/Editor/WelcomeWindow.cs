...
namespace Gentleland.Utils.SteampunkUI
{
    public class WelcomeWindow : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            {
                GUI.DrawTexture(
                    new Rect(position.width / 2 - imageWidth / 2, margin, imageWidth, imageHeight),
                    image,
            ...
            GUILayout.BeginArea(
                new Rect(
                    ...
                    position.height - imageHeight + 2 * margin
                )
            );
...
CEO - Gentleland",
            ...
            if (GUILayout.Button("Documentation.pdf"))
            {
                ...
                {
                    Application.OpenURL(
                        Application.dataPath + "/../" + AssetDatabase.GUIDToAssetPath(s[0])
                    );
                }
                else
                {
                    Debug.Log(
                        "Gentleland - " + PackageSettings.packageName +
                        " : Couldn't Find Documentation file it is either deleted or renamed"
                    );
                }
            }
